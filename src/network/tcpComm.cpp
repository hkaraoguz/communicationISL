...
/*tcpComm::tcpComm(robot* rbt)
{
	...
	for(int i = 0; i < MAX_CLIENT_NUM; i++){
		myClientSlots[i] = false;
		myClients[i] = NULL;
	}

	for(int i = 0; i < MAX_REMOTE_CLIENT_NUM; i++){
		remoteClientSlots[i] = false;
		remoteClients[i] = NULL;
	}
...
{
	/*for(int i = 0; i < MAX_CLIENT_NUM; i++) {
		if(myClients[i] != NULL){} //myClients[i]->deleteLater();
	}

	for(int i = 0; i < MAX_REMOTE_CLIENT_NUM; i++) {
		if(remoteClients[i] != NULL){} //remoteClients[i]->deleteLater();
	}*/
	
	//delete[] remoteClients;
}
void tcpComm::getClientImageReceived(const QImage& image){

	emit(sendClientImage(image));
...
void tcpComm::getClientDisconnected(int id, int clientType){
	...
	/*	if(remoteClients[id]->getClientHostName() == "localhost") {
			
			myClientSlots[id] = false;
			...
			getClientInfo(clientInfo , MY_CLIENT);

			myClients[id]->socket->abort();

			myClients[id]->deleteLater();
		}
		else if(remoteClients[id]->getClientIP() == "127.0.0.1"){
